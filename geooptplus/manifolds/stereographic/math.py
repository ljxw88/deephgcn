...
def _weighted_midpoint(
    ...
    numerator = (gamma * weights * xs).sum(reducedim, keepdim=True)
    two_mean = numerator / clamp_abs(denominator, 1e-10)
...
def _weighted_midpoint_bmm(
    ...
    numerator = torch.matmul(weights, gamma * xs)
    two_mean = numerator / denominator.clamp_min(1e-10) ## instead of clamp_abs
...
def _weighted_midpoint_spmm(
    ...
    gamma = _lambda_x(xs, k=k, dim=-1, keepdim=True)
    denominator = torch.sparse.mm(weights.abs(), gamma - 1)
    numerator = torch.sparse.mm(weights, gamma * xs)
    two_mean = numerator / denominator.clamp_min(1e-10)
    a_mean = two_mean / (1. + (1. + k * two_mean.pow(2).sum(dim=-1, keepdim=True)).sqrt())
